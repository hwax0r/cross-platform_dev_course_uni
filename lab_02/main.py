...
def my_walker(target_directory: str):
    """
        ...
        for file in files:
            ...
            if local_path != '':
                ...
                if path_to_file not in roots.keys():
                    roots.update({path_to_file: current_path[len(target_directory):]+'/'})

                new_filename = f'{path_to_file}_{file}'
                shutil.move(f'{current_path}/{file}', f'{target_directory}/{new_filename}')
    ...
    roots_report = open(f'{target_directory}/work_report.txt', 'a')
    for key, value in sorted(roots.items()):
        print("{}: {}".format(key, value), file=roots_report)


def main():
    target_dir: str = os.getcwd() + '/dir_after_script'
    my_walker(target_dir)
